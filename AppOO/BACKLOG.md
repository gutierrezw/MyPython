...
| ~~57~~ | ~~**IA/Plan**~~ | ~~**"Riesgos del Plan"** — panel UI editable para los parámetros de la misión del agente: meta capital, objetivo ingresos pasivos, escalado de objetivo, leverage máximo tolerado, pérdida máxima aceptable. Editable desde Misión IA / Editar Plan sin reiniciar app.~~ | ~~Media~~ |
| 🧪 58 | **Stock/Crypto — Índices** | Índice Stock/Crypto pasa de ratio `value/costo_base` a TWR real con flujos de capital (`daily_twr` + `select_bbva_ars_cashflows` generalizado con `categoria`/`cuentas`). Umbral de salto imposible 50% (vs 30% BBVA.ARS). No requiere reconstrucción de datos — cálculo corre en memoria en `performa_asset()`. **En observación** — validar en UI tras reinicio. | Media |
| ✅ 59 | **Stock/Crypto — Preservation** | **Diagnóstico corregido:** no era un hang — `py-spy dump` en vivo confirmó el thread activo, idle en `time.sleep()`. Causa real: `self._preservation_logger` apuntaba a un logger (`"Agente.Preservation"`) nunca registrado en `Class_debugging.py` (sin `FileHandler`) — 13 puntos de log (posiciones cargadas, evaluación, STOP, errores) se perdían silenciosamente; solo "config cargada"/"REVISIÓN" eran visibles (usan `_log_stock`). Fix: alias a `_log_stock`. Confirmado en logs en vivo (`REVISIÓN → posiciones cargadas → evaluación` completo). Detalle en `design-preservation.md` (sección Debugging). | Alta |
