...
| 58 | **Infraestructura/Sesión** | Editor de parámetros desde panel Sesión — método `_edit_session_params(vehiculo)` reutilizable por vehículo. Expone en UI los campos de `parameters` (preservation, gains_capture, agente_ia) hoy solo editables directo en BD. Diseño vehicle-aware: mismo método para Stock, Crypto, BotCrypto. Prerequisito: tener primero el editor de Gestión (#57) como referencia de patrón. | Media |
