...
def  datos_yfinance(symbol, vehiculo, read, ix):
    ...
    update = False
    info.update({symbol: {"activos": activos,              # almacena yf.Ticker.info()
                          "datos": datos,                  # almacena yf.history()
                          "datos_tecnicos": indicadores,   # almacena datos técnicos
                          "update": update}})              # True: si contiene dividends
...
def tipo_sell(indicadores, read, ix):
    ...
    if read[ix.index('stock')] == .0:
        tipo = '100%'
    elif read[ix.index('stock')] > .0:
        if opt <= .25:
            tipo = ' 25%'
        elif .25 > opt <= .33:
            tipo = ' 33%'
...
def oportunidades(file, update=False):
    ...
    if not df_sell.empty and update:
        for _, row in df_sell.iterrows():
            ...
            insert = ROportunidades.insertar_sell(row=row, 
                                                  tipo='sell', 
                                                  subtipo='gain', 
                                                  origen='modelo_sellv01', 
                                                  tolerancia_roi=.50)
            items += 1
...
def app(account, insert): 
    ...
    book, ix = ROportunidades.select_booktrading(accion="desde_hasta", 
                                                account=account,
                                                fecha=hasta,
                                                hasta=hoy)
    ...
    with (open(path, mode='w', newline='') as file):
        ...
        for i, read in enumerate(book):
            ...
            if fecha_favilda and read[ix.index('codigo')] == 'C' and read[ix.index('gprealizadas')] > 0:
                ...
                # carga info()
                indicadores = datos_yfinance(symbol=read[ix.index('simbolo')], 
                                             vehiculo=read[ix.index('categoria')], 
                                             read=read, 
                                             ix=ix)
