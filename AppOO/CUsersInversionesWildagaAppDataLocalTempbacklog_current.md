...
| 46 | **Stock/Consenso** | CUSIP mismatch — VST, ABX, BABA, PHYS muestran "4 inconsistencias" en health bar: sin datos en `fund_holdings` → voto Net=0 (fix aplicado). Investigar causa raíz: cruzar `market.cusip` vs `fund_holdings.cusip`; ABX/BABA son ADRs extranjeros (CUSIP distinto), PHYS es trust canadiense. `Agente_AuditPortfolio` ya intenta resolver CUSIPs vía EDGAR — verificar si los resuelve o son estructuralmente irresolvibles | Baja |
...
| 🧪 48 | **IA/Modelos** | Módulo Sentimiento: `ConvergIA/Scanner_Sentimiento.py` + `ConvergIA/Interprete_Sentimiento.py` + `ConvergIA/ThemeMapper.py` + tablas `market_sentiment`/`market_sentiment_analysis` + `Agente_Sentimiento` (1h) + `Agente_InterpreteSentimiento` (1d) + voto `Sent` en Consenso. Keys Claude separadas por módulo: `ClaudeAPIS`/`ClaudeAPIE`/`ClaudeAPIC` en tabla sesion — **implementado, pendiente validar en producción** | Media |
