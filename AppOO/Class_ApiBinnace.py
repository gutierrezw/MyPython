...
class BinanceSpot(Spot):
    ...
    def get_flexible_loan_borrow(self, loanCoin="USDT", collateralCoin=None, amount=0.001):
        url_path = f"{self._base_url}/sapi/v2/loan/flexible/borrow"
        xparams = {
            "loanCoin": loanCoin,
            "collateralCoin": collateralCoin,
        ...
        try:
            ...
            self.logger.error(f"get_flexible_loan_borrow({collateralCoin}): {e}")
    ...
    def ltv_check_and_adjust(self, lconfig):
        ...
        ltv_lower = target * (1 - tolerance)   # target - 5% relativo → ej: 0.28 * 0.95 = 0.266
        ltv_upper = target * (1 + tolerance)   # target + 5% relativo → ej: 0.28 * 1.05 = 0.294

        resultado = self.get_flexible_loan_ongoing_orders()
        if not resultado:
            return []
        ...
        for row in rows:
            if float(row.get("currentLTV", 0)) == 0:
                continue  # sin préstamo activo — nada que gestionar
            ...
            # loanValueInUSD / collateralValueInUSD no siempre viene en la respuesta
            # fallback: totalDebt (en loan coin, válido cuando loanCoin=USDT)
            ...
            if estado in ("ALTO", "CRITICO"):
                ...
                # LTV por debajo del rango — retirar exceso de colateral para subir LTV hacia target
                colateral_obj_usd = loan_usd / target if target > 0 else 0
    ...
    def get_simple_earn_account(self):
        ...
        try:
            ...
            self.logger.error(f"get_simple_earn_account(): {e}")
