        ...
        try:
            if not self.IClient.authenticated:
                ...
                if " Conexión   :" in self.resumen:
                    self.resumen[" Conexión   :"] = "ONLINE (yf)"
...
class DashMain:
    def __init__(self):
        ...
        # botón semáforo — modo operación de Agente_ClaudeIA (Capa 4): OBSERVACION / SUPERVISADO / AUTONOMO
        _AGENTE_MODOS = {
    ...
    def _splash_open(self, total=12):
        self.root.withdraw()
        win = tk.Toplevel()
        win.overrideredirect(True)
        win.attributes("-topmost", True)
