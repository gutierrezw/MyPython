...
class screener_board(tk.Frame):
    ...
    def widgets_screener(self):
        ...
        win1 = ttk.Frame(self, padding=(6, 6, 12, 12), style='TFrame')
        win2 = ttk.Frame(self, padding=(6, 6, 12, 12), style='W.TFrame')

        tree = ttk.Treeview(win1, columns=ctree,  height=24, style='Treeview')
        ...
        entry = ttk.Entry(win1, width=50, justify="left", font=("Arial", 24))
        ...
        tipo = tk.StringVar()
        tipo.set(s_tipo)
        tipo.trace("w", radiobutton_select)
        ...
        beta = tk.StringVar()
        beta.set(s_beta)
        beta.trace("w", radiobutton_select)
        ...
        mkt_c = tk.StringVar()
        mkt_c.set(s_market)
        mkt_c.trace("w", radiobutton_select)
        ...
        tree.grid(column=0, row=0, columnspan=2, rowspan=4, sticky=(N, S, E, W))
        h_scroll.grid(column=0, row=5, columnspan=2, sticky=E + W)
        v_scroll.grid(column=3, row=0, rowspan=4, sticky=N + S)

        search.grid(column=0, row=6, sticky=E, pady=50)
        entry.grid(column=1, row=6, columnspan=2, pady=50)
        entry.bind("<KeyRelease>", radiobutton_select)

        # set position of all above objects by grid  win2
        win2.grid(column=1, row=0, sticky=(N, S, E, W))
        tp1.grid(column=0, row=0, sticky=W, pady=5)
    ...
    def update_screener(self):
        ...
        market, ix = select_market(account=apilocal['account'], tipo=s_tipo, country=s_country, sector=s_sector)
        # d_sector, d_country = sector_country(market, ix)
